...
Pairs naturally with [redundo](https://github.com/CogentWizards/redundo), which reads this plugin's output directly with no collector step.

## Pricing data

Every `llm.call` span carries a cost estimate from this plugin's own bundled pricing snapshot. Refresh it any time:
...
```bash
npm install
npm run build       # compiles src/ into dist/, this is what actually ships
npm test            # compiles src/ and test/ into dist-test/, runs node --test against it
```
